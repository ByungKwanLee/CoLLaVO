...
class PLoRA(nn.Module):
    ...
    def __init__(self,
        ...
        self.Plora_seg_A = nn.Linear(
            in_features, self.lora_r, bias=False, device=device, dtype=dtype)
        self.Plora_seg_B = nn.Linear(
            self.lora_r, out_features, bias=False, device=device, dtype=dtype)
            ...
            if torch.sum(im_mask) > 0:
                ...
                if isinstance(seg_mask, torch.Tensor):
                    res[im_mask] += self.Plora_B(
                        self.Plora_A(
                            self.lora_dropout(part_x))) * self.lora_scaling + self.Plora_seg_B(
                        self.Plora_seg_A(
                            self.lora_dropout(seg_mask.view(-1, 4096)))) * self.lora_scaling
                else:
                    res[im_mask] += self.Plora_B(
                        self.Plora_A(
