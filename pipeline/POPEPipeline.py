        ...
        with torch.no_grad():
            ...
            for idx, batch in prog_bar:
                ...
                collavo_inputs = model.collavo_model.eval_process(images=torch.stack([x['image'] for x in batch]), 
                                                                    prompts=[f"{x['question']}\nAnswer the question using a single word or phrase." for x in batch], 
                ...
                with torch.inference_mode():
                    generate_ids = model.collavo_model.generate(**collavo_inputs, do_sample=False, num_beams=3, max_new_tokens=5, use_cache=True)
