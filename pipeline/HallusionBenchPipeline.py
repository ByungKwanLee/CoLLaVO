        ...
        with torch.no_grad():
            ...
            for idx, batch in prog_bar:
                ...
                collavo_inputs = model.collavo_model.eval_process(images=torch.stack([x['image'] for x in batch]), 
                                                                    prompts=[f"{x['question']}" for x in batch], 
                ...
                decoded_text = [x.split('assistant\n')[-1].strip().split('[UN')[0] for x in decoded_text]

                # Batch VQA evaluate process
                self.evaluator_total.process(batch, {'text': ['1' if x.lower().find('yes') != -1 else '0' for x in decoded_text]})
