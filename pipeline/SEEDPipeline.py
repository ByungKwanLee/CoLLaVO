    ...
    def evaluate_model(
        ...
        with torch.no_grad():
            ...
            for idx, batch in prog_bar:
                ...
                options = []
                for x in batch:
                    option = [x['choice_a'], x['choice_b'], ]
                    option_prompt = f'A. {option[0]} B. {option[1]} '  # noqa
                    if 'choice_c' in x:
                        option.append(x['choice_c'])
                        option_prompt = option_prompt + f'C. {option[2]} '
                    if 'choice_d' in x:
                        option.append(x['choice_d'])
                        option_prompt = option_prompt + f'D. {option[3]} '
                    if 'choice_e' in x:
                        option.append(x['choice_e'])
                        option_prompt = option_prompt + f'E. {option[4]} '
                    options.append(option_prompt.strip())

                # CoLLaVO: llm preparation
                collavo_inputs = model.collavo_model.eval_process(images=torch.stack([x['image'] for x in batch]), 
                                                                    prompts=[f"{x['question']}\n{option}\nAnswer with the option's letter from the given choices directly." for x, option in zip(batch, options)], 
                ...
                self.evaluator_total.process(batch, {'text': [x.split('assistant\n')[-1].strip().split('[UN')[0] for x in decoded_text]})
